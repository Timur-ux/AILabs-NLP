...
- /zero-shot: POST запрос определения спама по технике zero-shot
Тело запроса:
```json
{
	"message": string
...
```
- /cot: POST запрос определения спама по технике cot
- /few-shot: POST запрос определения спама по технике few-shot
- /cot-few-shot: POST запрос определения спама по технике cot-few-shot
Тело запроса:
```json
{
	"message": string
...
После запуска backend будет доступен на 8000 порту
...
# Анализ техник промптинга

Для каждой техники промптинга я использовал 100 обычных сообщений и 100 сообщений спама. Они выбирались случайно, поэтому должны давать хорошую аппроксимацию всего датасета.

Я не использовал весь датасет т.к. его прогон занял бы часы на gpu и дни на cpu. 200 сообщений на видеокарте NVidia RTX 4050 mobile обрабатываются за ~20 минут, на CPU Intel Core i5 13 поколения ~60 минут.
...
```
Check whether following email message is spam or not spam: \'{}\'. Output must be in plain JSON format with 2 fields. First must be "verdict" with value 1 if message is spam or 0 if it is not. Second must be "reasoning" with describing why you decide mark message as spam or not spam.
```
...
|-------------|----------|-----------|--------|----|
| Zero-shot | 0.4797 | 0.4736 | 0.4787 | 0.4761|
| CoT | 0.4731 | 0.0515 |  0.2631 | 0.0862 |
| Few shot | 0.5279 | 0.1442 | 0.5769 | 0.2307|
| CoT + Few shot | 0.5194 | 0.2242 | 0.5106 | 0.3116|

# Выводы

По таблице видно, что предсказания модели ведут себя абсолютно случайно. При подробном анализе вывода модели выяснилось, что вне зависимости от того, что она написала в reasoning, в поле verdict может быть как истина так и ложь. Предположительно модель сначала генерировала вердикт, а только потом писала рассуждения, хотя я и указывал делать наоборот. Также к этому может приводить и малый размер модели.

Как инженерный проект работа выполнена полностью. Для дальнейших исследований желательно изменить модель на более комплексную, для чего необходимо изменить название модели в файлах ./ollama/serveAndPull.sh и ./server/main.py
